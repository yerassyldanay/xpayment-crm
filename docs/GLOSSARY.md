...
- **Persona / "soul" vs. code-owned skeleton** — The editable identity/guardrails (soul, in `assistant.json`) vs. the non-editable FRAME — the JSON contract, assembly order, and hard rules (skeleton, in code). See [03 · file shapes](03-content-and-data.md#file-shapes).
- **Guardrails** — The editable must/must-not rules shaping the bot's behavior (block `[C]`). See [03 · file shapes](03-content-and-data.md#file-shapes).
...
- **Topic-organized KB** — Knowledge grouped into answerable subjects (`knowledge/*.md`), authored in both languages, all loaded into the prompt. See [03 · file shapes](03-content-and-data.md#file-shapes).
- **LLM-as-selector** — Instead of search, the whole catalog is shown to the model and it returns the items it wants by name (Decision 7). See [03 · file shapes](03-content-and-data.md#file-shapes).
- **Asset catalog** — The full list of media (`media.json`) rendered into the prompt as a menu (`ref | kind | topic | description`). See [02 · Prompt assembly](02-assistant-brain.md#prompt-assembly).
- **`ref`** — The stable slug naming one media asset (e.g. `add_cashier_video`); the menu key the model returns. See [03 · file shapes](03-content-and-data.md#file-shapes).
...
- **Binaries vs. metadata** — Media **files** live in the `xpayment-content` repo (served by URL); only **metadata** lives in `media.json`. See [03 · file shapes](03-content-and-data.md#file-shapes).
- **Vector index / embeddings / RAG** — Semantic-search machinery for retrieving relevant chunks at scale — **deliberately avoided** (Decision 7) because the corpus fits in one cached prompt; kept as a documented future option. See [03 · the content repo](03-content-and-data.md#the-content-repo-xpayment-content).
...
- **Draft → publish → rollback** — The config lifecycle via git: a branch is a draft, merge-to-`main` publishes, `git revert` rolls back. See [08 · the lifecycle maps onto git](08-admin-ui.md#the-lifecycle-maps-onto-git).
- **Playground** — A CLI dry-run: try a message and see the draft, media, and profile without sending or touching a real conversation. See [08 · Playground CLI](08-admin-ui.md#playground-cli--test-before-you-commit).
...
- **Standalone service** — The brain runs as its own Go service/repo (`xpayment-crm`), **stateless (no database)**, with its own image and config, reusing the main repo's conventions (Decision 12). See [04 · Repo layout](04-service-and-deployment.md#repo-layout).
...
- **Content checkout** — The `xpayment-content` repo mounted (read-only volume) or cloned into the brain at boot, then loaded into the snapshot. See [04 · content checkout & reload](04-service-and-deployment.md#content-checkout--reload).
...
- **Backups** — Scheduled dumps of Chatwoot's Postgres (the system of record); the brain is stateless, so its config lives in git, not a DB. See [01 · Backups](01-infrastructure.md#backups).
...
- **Content loader/validator test** — Unit tests asserting the snapshot loader rejects a missing media file or an unresolved price token (no DB needed). See [07 · Integration tests](07-testing-and-evals.md#integration-tests).
...
There is **no admin API or cross-service auth** in v1 (Decision 12) — config access control is git permissions on `xpayment-content`.
...
- **Reload-webhook secret** — The HMAC secret on the GitHub push webhook that triggers a content reload. See [06 · GitHub reload webhook](06-api-and-contracts.md#github-reload-webhook).
...
- **Content repo** — `xpayment-content`: the git repo holding `assistant.json`, `pricing.json`, `media.json`, `knowledge/*.md`, and `media/`; the brain's config source (Decision 2). See [03 · the content repo](03-content-and-data.md#the-content-repo-xpayment-content).
- **Snapshot** — The immutable in-memory load of the content repo the brain serves from; rebuilt and atomically swapped on reload. See [03 · the in-memory snapshot](03-content-and-data.md#the-in-memory-snapshot).
- **Atomic swap** — Replacing the live snapshot pointer only after the new one validates, so a bad edit never goes live. See [03 · validate on load](03-content-and-data.md#validate-on-load-fail-loudly).
- **Reload-on-push** — A GitHub push webhook → `git pull` → rebuild + validate → swap. See [04 · content checkout & reload](04-service-and-deployment.md#content-checkout--reload), [06](06-api-and-contracts.md#github-reload-webhook).
- **GitOps lifecycle** — Branch/working-copy = draft, merge-to-`main` = publish, `git revert` = rollback, `git log`/`blame` = audit, PR = review (Decision 12). See [08 · the lifecycle maps onto git](08-admin-ui.md#the-lifecycle-maps-onto-git).
- **Validation-on-load** — The loader fails loudly if a media `file` is missing or a price token doesn't resolve; the bad snapshot is rejected. See [03 · validate on load](03-content-and-data.md#validate-on-load-fail-loudly).
- **Playground CLI** — Builds a snapshot from the working copy (incl. uncommitted edits) and runs `HandleMessage` with mocks, to test before committing. See [08 · Playground CLI](08-admin-ui.md#playground-cli--test-before-you-commit).
- **Git LFS** — Large-file storage for video media binaries kept in git (vs object storage). See [03 · the content repo](03-content-and-data.md#the-content-repo-xpayment-content).
